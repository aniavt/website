...
export class FindNodesByTagUseCase {
    constructor(
        private readonly vaultService: VaultService,
        private readonly userRepository: UserRepository,
    ) {}

    async execute(requesterId: string | null, tagId: string): Promise<Result<VaultNodeEntity[], VaultError>> {
        if (!tagId) {
        ...
        const result = await this.vaultService.findNodesByTagId(tagId);
        if (result.isError()) {
            return result;
        }
