...
export class RenameTagUseCase {
    ...
    async execute(requesterId: string, tagId: string, newName: string): Promise<Result<VaultNodeTagInfoEntity, VaultError>> {
        ...
        return await this.vaultService.renameTag(tagId, newName);
    }
}
