...
export class SetThumbnailUseCase {
    constructor(
    ...
    async execute(
        ...
        if (!requester.hasPermission({ type: "vault", permission: VaultPermission.UPDATE_NODE })) return err("vault_not_authorized");

        return await this.vaultService.setThumbnail(nodeId, thumbnailFileId);
    }
}
