...
export class GetSourcesForNodeUseCase {
    constructor(
    ...
    async execute(
        ...
        if (!node) return err("vault_node_not_found");

        if (node.isPublic) return await this.vaultService.getSourcesForNode(nodeId);
        ...
        return await this.vaultService.getSourcesForNode(nodeId);
    }
}
