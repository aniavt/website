...
export class BootstrapRootUseCase {
    constructor(
    ...
    async execute({ username, password }: BootstrapRootInput): Promise<Result<UserDto, UserError>> {
        const existingRoots = await this.userRepository.findRootUsers({ limit: 1 });
