...
describe("RestoreFaqItemUseCase", () => {
  async function setup() {
    ...
    await users.save(
      createUser({
        id: "admin",
        grants: [{ type: "faq", permission: FAQPermission.RESTORE_FAQ }],
      }),
    );
    ...
    const uc = new RestoreFaqItemUseCase(faqItem, faqText, faqHistory, users, idGen, tx);
    return { faqItem, faqHistory, users, uc };
  }

  test("restores deleted item and appends history", async () => {
    const { faqItem, faqHistory, uc } = await setup();
    const dto = expectOk(await uc.execute("admin", "faq-1"));
    expect(dto.isActive).toBe(true);
    expect(dto.lastAction).toBe("restore");
  ...
  test("unauthorized / not found / invalid transition", async () => {
    ...
    await faqItem.save(createFaqItem({ id: "active", queryId: "q-1", answerId: "a-1" }));
    expectErr(await uc.execute("admin", "active"), "faq_invalid_transition");
  });
});
